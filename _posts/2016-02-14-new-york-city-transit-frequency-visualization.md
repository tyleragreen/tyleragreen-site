...
Stop the buses! Hold the phone! I now have visual proof that buses and subways in the Big Apple run more often on Fridays than Saturdays. How insightful, right? Okay, so maybe not, but I still enjoyed making a New York City transit frequency visualization using [Transitland](https://transit.land/) and [Mapbox](https://www.mapbox.com/).
...
I was inspired to dig into Transitland by this [similar frequency visualization for San Francisco](https://gist.github.com/irees/272e5dc57614cab595a0). We both use the stops and schedule\_stop\_pairs API endpoints to calculate how often the "edge" between any two consecutive transit stops is visited in a given time frame.
...
GeoJSON is a standard JSON variant that holds geographical information, such as points and line segments. In addition to its required fields, I loaded the GeoJSON output files with styling from Mapbox's [simplestyle-spec](https://github.com/mapbox/simplestyle-spec/tree/master/1.1.0) based on the frequency for that line segment. Mapbox interprets these "properties" fields when displays the data on a map.
...
As the JSON Transitland interface language-agnostic, any scripting language could be used. Ruby is by far my favorite, so I stuck with what I know. You can view the visualization in [my GitHub repository](https://github.com/tyleragreen/frequency-visualization).  The code is divided into an HTML front-end and Ruby back-end, though they do not connect directly. A few ideas I have for the future of this project:

  * The TransitlandAPIReader class could be generalized into a gem with a decent test suite, similar to [one Transitland used to maintain and intends to bring back](https://github.com/transitland/transitland-ruby-client).
